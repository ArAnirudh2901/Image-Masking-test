...
const KEY_HINT = '<kbd>0</kbd> reset zoom <span class="sep">·</span> <kbd>⌘Z</kbd> undo'

const HINTS = {
    mouse: `wheel zoom <span class="sep">·</span> middle-drag pan <span class="sep">·</span> ⌥-click removes <span class="sep">·</span> ${KEY_HINT}`,
    trackpad: `pinch zoom <span class="sep">·</span> two-finger scroll pans <span class="sep">·</span> ⌥-click removes <span class="sep">·</span> ${KEY_HINT}`,
    touch: 'pinch to zoom <span class="sep">·</span> two fingers pan',
}
...
export const createGestures = ({ stage, frame, surface, hint, readout, active, onGestureStart, onZoom }) => {
    ...
    // Every binding is tracked so a host that unmounts the stage can take them
    // with it. Without `destroy` each remount leaves a live wheel and keydown
    // handler behind, still transforming an element that is gone.
    const bound = []
    ...
    // host reads ⌥-click as exclude, so it only has to not open a menu.
    const noMenu = (e) => e.preventDefault()

    on(stage, 'wheel', onWheel, { passive: false })
    on(stage, 'pointerdown', onDown, { capture: true })
    on(stage, 'pointermove', onMove, { capture: true })
    on(stage, 'pointerup', onUp, { capture: true })
    on(stage, 'pointercancel', onUp, { capture: true })
    on(window, 'keydown', onKey)
    on(window, 'keyup', onKeyUp)
    on(window, 'resize', apply)
    on(readout, 'click', reset)
    on(surface, 'contextmenu', noMenu)
    apply()  // sync the readout's hidden flag before the first gesture
